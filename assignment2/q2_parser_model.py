...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embeddings,self.input_placeholder)##return ith row of embeddings tensor according
                                                                              ## to self.input_pkaceholder's num (i)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("predicted"):
            ...
            b1 = tf.Variable(tf.truncated_normal([self.config.hidden_size,]))
            U = xavier_initializer([self.config.hidden_size , self.config.n_classes])
            b2 = tf.Variable(tf.truncated_normal([self.config.n_classes]))

            h = tf.nn.relu(tf.matmul(x,W) + b1)
            h_drop = tf.nn.dropout(h,keep_prob=self.dropout_placeholder) ###The keep probability should be set to the value of self.dropout_placeholder
    ...
    def add_loss_op(self, pred):
        ...
        loss += self.config.l2_regular_arg * tf.nn.l2_loss(self.W)
