...
def softmax(x):
    ...
    #out = tf.zeros_like(x)
    #tmp = tf.exp(x)
    #out = tmp / tf.reduce_sum(tmp,axis=1,keep_dims=True)
    ###built-in
    out = tf.nn.softmax(logits=x)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    # s = tf.mod(yhat,tf.to_float(y))
    # print  s.shape
    # out = -tf.log(tf.reduce_sum(s))
    # print  out.shape
    ######built-in
    out = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=y,logits=yhat,dim=-1))
    ### END YOUR CODE
