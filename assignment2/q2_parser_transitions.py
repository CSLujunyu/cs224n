...
def minibatch_parse(sentences, model, batch_size):
    ...
    for item in sentences:
        partial_parses.append(PartialParse(item))
    ...
    while len(unfinished_parses) > 0:
        ...
        while len(minibatch) > 0:
            ...
            minibatch = [item for item in minibatch if len(item.stack)>1 or len(item.buffer)>0]##update the minibatch,sentences have different length
